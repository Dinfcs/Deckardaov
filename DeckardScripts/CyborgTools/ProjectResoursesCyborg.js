...
// @version      3.7.3
// @description  Visualización mejorada de recursos de proyectos / se corrije bug de hrf falsos
// @author
...
(function () {
    ...
    function createIframeWithTabs() {
       ...
       const capeMayPattern = /^https:\/\/cyborg\.deckard\.com\/listing\/NJ\/cape_may\/city_of_cape_may\/STR.*/;
        ...
        } else if (capeMayPattern.test(window.location.href)) {
            // If the current URL matches the Cape May pattern, use the specific parcel URL
            parcelUrl = 'https://cyborg.deckard.com/parcel/NJ/cape_may/_';
        } else {
            // Buscar primero el enlace con el texto correcto
            const allParcelsLink = Array.from(document.querySelectorAll('a[href*="/parcel/"]'))
                .find(link => link.textContent.trim() === "All parcels in the region");

            // Si no existe, buscar el enlace alternativo
            const parcelLink = allParcelsLink || document.querySelector('a[href*="/parcel/"]');

            console.log(parcelLink ? parcelLink.href : "No se encontró el enlace correcto");
